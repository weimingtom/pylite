...
{
	int status;
	py_object_t *py_status;
    assert_argc(argc, 1);
    py_status = argv[0]; 
    status = py_object_to_integer(py_status);
...
void py_sys_module_init(void)
{
    ...
    py_stdin = py_file_fopen("/dev/stdin", "r"); 
    py_stdout = py_file_fopen("/dev/stdout", "w"); 
    py_stderr = py_file_fopen("/dev/stderr", "w"); 
